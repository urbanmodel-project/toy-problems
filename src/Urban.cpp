...
const char* UrbanGetErrorString(UrbanErrorCode err) {
  switch (err) {
    ...
    case URBAN_ERR_NOT_INITIALIZED: return "not initialized";
    case URBAN_ERR_INTERNAL: return "internal error";
    default: return "unknown error";
  }
}

UrbanErrorCode UrbanSetLogger(UrbanType handle, UrbanLogFn fn, void* user_data) {
...
static void copy_1d_to_dual(const UrbanArrayD& arr, HostArray1DR8& host, Array1DR8& dev) {
  if (!arr.data) return;
  const size_t n = arr.size;
  if (host.extent(0) < (int)n) return; // simplistic bounds guard
  for (size_t i = 0; i < n; ++i) host(i) = static_cast<R8>(arr.data[i]);
...
UrbanErrorCode UrbanSetInputs(UrbanType handle, const UrbanInputs* in) {
  ...
  try {
    ...
    // Shortwave: distribute to direct beam band 0; set diffuse to zero
    {
      ...
      // Set diffuse to zero
      Kokkos::deep_copy(handle->bundle.input.SdifHoriz, handle->bundle.input.SdifHorizH);
    }
...
UrbanErrorCode UrbanGetOutputs(UrbanType handle, UrbanOutputs* out) {
  ...
  try {
    ...
    if (!out->net_shortwave.data || out->net_shortwave.size < (size_t)n) return URBAN_ERR_INVALID_ARGUMENT;
    if (!out->net_longwave.data || out->net_longwave.size < (size_t)n) return URBAN_ERR_INVALID_ARGUMENT;
    if (!out->surface_flux.data || out->surface_flux.size < (size_t)n) return URBAN_ERR_INVALID_ARGUMENT;
...
UrbanErrorCode UrbanSetOptionInt(UrbanType, const char*, int) { return URBAN_SUCCESS; }
UrbanErrorCode UrbanSetOptionDouble(UrbanType, const char*, double) { return URBAN_SUCCESS; }
UrbanErrorCode UrbanSetOptionBool(UrbanType, const char*, bool) { return URBAN_SUCCESS; }

UrbanErrorCode UrbanCopyOutputs(UrbanType handle,
